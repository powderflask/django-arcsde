...
class TzTests(TestCase):
    def test_localize_naive(self):
        """ Test that a naive datetime from DB is interpreted as UTC. """
        dt = datetime.now()
        self.assertEqual(dt.tzinfo, None)
        local = tz.localize(dt)
        self.assertEqual(local.tzinfo, ZoneInfo(settings.TIME_ZONE))
    ...
    def test_delocalize_local(self):
        ...
        self.assertEqual(sde.tzinfo, ZoneInfo(settings.SDE_DB_TIME_ZONE))
    ...
    def test_delocalize_naive(self):
        """ Test that a naive datetime from system or user is interpreted as Local Time. """
        dt = datetime.now()
        self.assertEqual(dt.tzinfo, None)
        sde = tz.delocalize(dt)
        self.assertEqual(sde.tzinfo, ZoneInfo(settings.SDE_DB_TIME_ZONE))
